...
bool intersect_cylinder(const t_ray *ray, const t_cylinder *cylinder, float *t)
{
    ...
    float proj_origin = dot(ray->origin, axis);
    float proj_t0 = proj_origin + t0 * dot(ray->direction, axis);
    float proj_t1 = proj_origin + t1 * dot(ray->direction, axis);

    float half_height = cylinder->height / 2.0;

    if (proj_t0 < -half_height || proj_t0 > half_height) {
        if (proj_t1 < -half_height || proj_t1 > half_height) {
            return false;
        } else {
            *t = t1;
            return (*t >= 0);
