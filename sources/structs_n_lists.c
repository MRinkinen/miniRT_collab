...
/*   Updated: 2024/10/25 17:20:42 by tvalimak         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	free_cylinder(t_var *var, int obj_index)
{
	if (var->objects[obj_index].transform)
		free_matrix(cylinder.transform);
	if (cylinder->inverse_transform)
		free_matrix(cylinder->inverse_transform);
	if (cylinder->translation_matrix)
...
void	terminate_var_data(t_var *var, char *error)
{
	...
	if (error)
		printf("%s\n", error);
	while (obj_index < var->num_objects)
	{
		if (var->objects[obj_index].type == CYLINDER)
			free_cylinder(var->objects[obj_index], obj_index);
		if (var->objects[obj_index].type == PLANE)
			free_plane();
		if (var->objects[obj_index].type == SPHERE)
			free_sphere();
		obj_index++;
	}
}
...
*/
// cylinder uses malloced matrices:
// identity
// transform
// inverse
void	free_cylinder(t_cylinder *cylinder)
...
{
	int	obj_index;
	printf("in terminate var_data\n");

	obj_index = 0;
	if (error)
		printf("%s\n", error);

	printf("var->objects: %p\n", (void *)var->objects);
	printf("var->num_objects: %d\n", var->num_objects);

	while (var->objects && obj_index < var->num_objects)
	{
		printf("Object %d type: %d\n", obj_index, var->objects[obj_index].type);
		//if (var->objects[obj_index].type == 1)
		if (var->objects[obj_index].type == CYLINDER)
		{
			printf("in free cyl of the term_var_data_loop\n");
			free_cylinder(&(var->objects[obj_index].data.cylinder));
		}
		obj_index++;
	}
	// Free any additional allocations within var
	...
	printf("terminate var_data ended ok\n");
}
